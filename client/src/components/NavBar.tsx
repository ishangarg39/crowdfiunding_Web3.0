      ...
      <div className="lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]">
        ...
        <div className='sm:hidden flex justify-between item-center relative'>    
          <div className='w-[40px] h-[40px] rounded-[10px] bg-[#2c2f32] flex justify-center items-center cursor-pointer'>
            <img src={logo} alt="user" className="w-[60%] h-[60%] oblect-contain"/>
          </div>
          <img src={menu}
          alt="menu"
          ...
          <div className={`absolute top-[60px] right-0 left-0 bg-[#1c1c24] z-10 shawdow-secondary py-4 ${!toggleDrawer? '-translate-y-[100vh]':'translate-y-0'} transition-all duration-700`}>
            <ul className='mb-4'>
              {navlinks.map((link)=>(
                <li 
                key={link.name}
            ...
            <CustomButton 
              ...
              handleClick={()=>{
                if(address) navigate("create-campaign");
                  else{console.log("conn"+connect());connect();}
                }}
            />
            </div>
          </div>
